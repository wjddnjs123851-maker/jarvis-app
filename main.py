...
# --- [3. UI 설정] ---
st.set_page_config(page_title="JARVIS Prime v84.0", layout="wide")
now = datetime.utcnow() + timedelta(hours=9)

# CSS: 인덱스 열 숨기기 및 우측 정렬
st.markdown("""
<style>
...
if menu == "💰 자산 & 가계부":
    ...
    if not df_a.empty:
        ...
        with col1:
            ...
            if not df_pos.empty:
                ...
                df_pos["수량"] = pd.to_numeric(df_pos["수량"], errors='coerce').fillna(0.0)
                df_pos["평가액"] = pd.to_numeric(df_pos["평가액"], errors='coerce').fillna(0.0)
                st.data_editor(
                    df_pos[['항목', '수량', '단위', '평가액']],
                    ...
                    key="view_assets_top", disabled=True
                )
        with col2:
            st.markdown("#### 🔴 부채 및 카드값")
            df_neg = pd.DataFrame(d_rows)
            if not df_neg.empty:
                df_neg["수량"] = pd.to_numeric(df_neg["수량"], errors='coerce').fillna(0.0)
                df_neg["평가액"] = pd.to_numeric(df_neg["평가액"], errors='coerce').fillna(0.0)
                st.data_editor(
                    df_neg[['항목', '수량', '단위', '평가액']],
                    use_container_width=True,
                    column_config={
    ...
    with t2:
        ...
        cal_url = st.text_input("개인 구글 캘린더 '이 사이트에 게시' URL을 입력하세요", 
                                ...
                                help="구글 캘린더 설정 -> 내 캘린더 설정 -> 캘린더 통합 -> 이 사이트에 게시 URL (HTTPS 링크)")
        if cal_url:
    ...
    if not df_p.empty:
        df_v = df_p.copy()
        if len(df_v.columns) > 1: df_v = df_v.drop(df_v.columns[1], axis=1) # 불필요 열 제거
