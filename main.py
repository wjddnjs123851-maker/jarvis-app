...
EXPENSE_CATS = ["식비(집밥)", "식비(외식)", "식비(배달)", "식비(편의점)", "생활용품", "건강/의료", "기호품", "주거/통신", "교통/차량", "금융/보험", "결혼준비", "경조사", "기기지출"]
...
# [복구] 보스가 알려준 영양소 순서 고정
DAILY_GUIDE = {
    "칼로리": {"val": 2000.0, "unit": "kcal"},
    "단백질": {"val": 150.0, "unit": "g"},
    "탄수화물": {"val": 300.0, "unit": "g"},
    "지방": {"val": 65.0, "unit": "g"},
    "콜레스테롤": {"val": 300.0, "unit": "mg"},
    "나트륨": {"val": 2000.0, "unit": "mg"},
    "당": {"val": 50.0, "unit": "g"},
...
if menu == "투자 & 자산":
    ...
    # [구조 변경] 분류 삭제, 수량/유통기한 위주, 교체주기를 하단으로 병합
    ...
    st.write("🛒 **식재료 현황 (수량 및 유통기한 중심)**")
    inv_display = st.session_state.inventory.copy()
    inv_display.index = range(1, len(inv_display) + 1)
    edited_inv = st.data_editor(inv_display, num_rows="dynamic", use_container_width=True, key="inv_no_cat")
    ...
    if 'supplies' not in st.session_state:
        st.session_state.supplies = pd.DataFrame([
            ...
            {"품목": "베개커버", "최근교체일": "2026-02-12", "주기(일)": 7}
        ])
    
    supp_display = st.session_state.supplies.copy()
    supp_display.index = range(1, len(supp_display) + 1)
    edited_supp = st.data_editor(supp_display, num_rows="dynamic", use_container_width=True, key="supp_bottom")
    ...
    with col_update:
        sel_item = st.selectbox("업데이트할 품목", st.session_state.supplies['품목'].tolist())
        if st.button("오늘 날짜로 교체 완료"):
