...
# --- [2. 유틸리티] ---
def format_krw(val): return f"{int(val):,}" + "원" # 원 단위 표기 필수
def to_numeric(val):
...
with t_c1: st.markdown(f"### 🗓️ {datetime.now().strftime('%Y년 %m월 %d일')} | 🌡️ 8°C ☀️ 맑음")
...
if menu == "투자 & 자산":
    ...
    if 'supplies' not in st.session_state:
        st.session_state.supplies = pd.DataFrame([
            {"품목": "칫솔", "최근교체일": "2026-01-15", "주기(일)": 30}, {"품목": "면도날", "최근교체일": "2026-02-01", "주기(일)": 14}, {"품목": "이불빨래", "최근교체일": "2026-02-08", "주기(일)": 14}
        ])
