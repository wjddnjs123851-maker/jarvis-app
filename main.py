...
if menu == "투자 & 자산":
    ...
    with f_c2: cats = ["식비(집밥)", "식비(외식)", "생활용품", "주거/통신", "결혼준비", "가전/가구", "기타"] if t_choice != "수입" else ["급여", "금융소득", "기타"]; c_choice = st.selectbox("카테고리", cats)
    ...
    st.header("영양 분석 리포트")
    st.info(f"목표: 5월 30일 결혼식 전 체중 감량 (현재: {in_w:.2f}kg)")
