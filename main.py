...
st.set_page_config(page_title="JARVIS v62.0", layout="wide")
st.markdown(f"""
    <style>
...
def send_to_sheet(d_date, d_hour, d_type, cat_main, cat_sub, content, value, method):
    # 날짜와 시간을 조합하여 시트 기록
...
# 메인 화면
st.markdown(f"### {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | JARVIS Prime (White)")
...
if menu == "투자 & 자산":
    ...
    with st.sidebar:
        st.subheader("데이터 입력")
        sel_date = st.date_input("기록 날짜", value=datetime.now() + timedelta(hours=9))
        sel_hour = st.slider("시간 (시 단위)", 0, 23, (datetime.utcnow() + timedelta(hours=9)).hour)
        ...
        if st.button("시트 데이터 전송"):
            if a_input > 0:
                if send_to_sheet(sel_date, sel_hour, t_choice, c_main, c_sub, content, a_input, method_choice):
                    st.success(f"{sel_hour}시 내역 기록 완료!"); st.cache_data.clear(); st.rerun()
