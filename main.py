...
st.set_page_config(page_title="JARVIS v2.1", layout="wide")
st.title("JARVIS : REAL-TIME DASHBOARD") # 이모지 삭제
...
with a1:
    ...
    total_row_a = pd.DataFrame([{"항목": "TOTAL ASSETS", "금액": f"{total_a:,.0f}원"}])
    ...
    total_s = df_s['수량'].sum()
    total_row_s = pd.DataFrame([{"종목": "TOTAL QUANTITY", "수량": total_s}])
...
with a2:
    ...
    total_row_d = pd.DataFrame([{"항목": "TOTAL DEBT", "금액": f"{total_d:,.0f}원"}])
    df_d_disp = pd.concat([df_d_disp, total_row_d], ignore_index=True)
    df_d_disp.index = df_d_disp.index + 1
    st.table(df_d_disp)
    
    st.metric("NET WORTH (REAL-TIME)", f"{total_a - total_d:,.0f}원")

# --- SECTION 3: 관리 주기 & 인벤토리 ---
st.header("LIFECYCLE & KITCHEN")
l1, l2 = st.columns(2)
with l1:
    life_rows = []
    for item, info in MY_DATA["lifecycle"].items():
        ...
        status = "NEED CHECK" if rem <= 0 else "NORMAL"
        life_rows.append({"항목": item, "상태": status, "남은일수": f"{rem}일"})
