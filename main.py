...
st.set_page_config(page_title="자비스 v4.4", layout="wide")

# CSS: 인덱스 숫자 전용 특대화 및 데이터 정렬
st.markdown("""
    <style>
    /* 1. 표의 첫 번째 열(번호)을 텍스트보다 크게 만듦 */
    [data-testid="stTable"] td:nth-child(1), 
    [data-testid="stTable"] th:nth-child(1) {
        font-size: 38px !important;  /* 숫자를 제목보다 큰 38px로 설정 */
        font-weight: 900 !important;
        color: #FF4B4B !important;   /* 눈에 확 띄는 자비스 레드 */
    ...
    /* 3. 섹션 헤더 크기 */
    h2 { font-size: 30px !important; font-weight: bold; border-left: 10px solid #FF4B4B; padding-left: 15px; }

    /* 4. 숫자 데이터 우측 정렬 */
    td:nth-child(2), td:nth-child(3) { text-align: right !important; }
    
    /* 5. 메트릭(순자산 등) 강조 */
    [data-testid="stMetricValue"] { font-size: 36px !important; font-weight: bold; }
    </style>
...
with f_col1:
    assets = [{"항목": "현금", "금금": st.session_state.cash}]
    ...
    st.table(df_a.assign(순번=range(1, len(df_a)+1)).set_index('순번'))
with f_col2:
...
with l_col1:
    ...
    st.table(pd.DataFrame(l_rows).assign(순번=range(1, 5)).set_index('순번'))
with l_col2:
