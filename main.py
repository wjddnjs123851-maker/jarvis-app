...
# --- [3. 메인 설정] --- (변동 없음)
st.set_page_config(page_title="JARVIS v34.0", layout="wide")
...
if menu == "투자 & 자산":
    ...
    # 2. 생활용품 교체주기 (동적 편집 버전)
    st.subheader("⏰ 생활용품 교체주기 관리 (자유 편집)")
    ...
    # 식자재와 마찬가지로 자유롭게 추가/삭제/수정 가능한 에디터
    edited_supp = st.data_editor(supp_display, num_rows="dynamic", use_container_width=True, key="supp_editor_final")
    ...
    with sc1:
        sel_item = st.selectbox("교체 완료한 품목", st.session_state.supplies['품목'].tolist())
    with sc2:
        st.write(""); st.write("")
