...
st.set_page_config(page_title="자비스 v6.0", layout="wide")
...
with st.sidebar:
    ...
    with st.form("main_input"):
        ...
        if st.form_submit_button("반영"):
            # 입력 데이터 로그 생성 (새 항목 포함)
            entry = {
                "날짜": datetime.now().strftime('%Y-%m-%d'), "시간": datetime.now().strftime('%H:%M'), 
            ...
            if "물" in meal_in: entry["수분"] = 500
                ...
                entry.update({"칼로리": 1120, "단백질": 50, "지방": 55, "탄수화물": 110, "식이섬유": 5, "나트륨": 1200, "콜레스테롤": 150, "당류": 12})
            elif meal_in: 
                entry.update({"칼로리": 600, "단백질": 25, "지방": 20, "탄수화물": 70, "식이섬유": 3, "나트륨": 800, "당류": 5})
...
# --- 2. 건강 및 영양 (상세 항목 포함) ---
st.header("2. 건강 및 영양 상세")
c1, c2, c3 = st.columns(3)
c1.metric("오늘 칼로리", f"{st.session_state.consumed['칼로리']} / 2000")
c2.metric("나트륨 섭취", f"{st.session_state.consumed['나트륨']} / 2000 mg")
...
for k, v in st.session_state.consumed.items():
    if k not in ['칼로리', '나트륨', '콜레스테롤']:
        ...
        nut_rows.append({"항목": k, "현황": f"{v}{unit}"})
